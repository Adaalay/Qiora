const seccionTelefono =
    document.getElementById("secuenciaTelefono");

const videoTelefono =
    document.getElementById("videoTelefono");

let animacionPendiente = false;

function actualizarVideoTelefono() {
    if (
        !seccionTelefono ||
        !videoTelefono ||
        !Number.isFinite(videoTelefono.duration)
    ) {
    ...
    const rect =
        seccionTelefono.getBoundingClientRect();

    const recorrido =
        seccionTelefono.offsetHeight -
        window.innerHeight;

    let progreso =
        -rect.top / Math.max(1, recorrido);

    progreso =
        Math.max(0, Math.min(1, progreso));

    videoTelefono.currentTime =
        progreso * (videoTelefono.duration - 0.05);
...
function solicitarActualizacionVideo() {
    ...
    requestAnimationFrame(
        actualizarVideoTelefono
    );
}

videoTelefono.addEventListener(
    "loadedmetadata",
    function () {
        videoTelefono.currentTime = 0.01;
        actualizarVideoTelefono();
    }
);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function hideLoading() {
        ...
        setTimeout(() => loader.remove(), 800); // elimina del DOM
    }
